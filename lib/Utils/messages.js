...
const generateWAMessageContent = async (message, options) => {
    ...
    var _p, _q;
    let m = {};
    if ('buttonsMessage' in message && !!message.buttonsMessage) {
        return { buttonsMessage: message.buttonsMessage };
    }
    if ('interactiveButtons' in message && !!message.interactiveButtons) {
        ...
        let hasMedia = false;
        let mediaHeader = {};
        if ('image' in message || 'video' in message || 'document' in message || 'audio' in message || 'sticker' in message) {
            hasMedia = true;
            const mediaResult = await (0, exports.prepareWAMessageMedia)(message, options);
            mediaHeader = mediaResult;
        }
        if ('text' in message) {
            ...
            if (hasMedia && Object.keys(mediaHeader).length > 0) {
                Object.assign(interactiveMessage.header, mediaHeader);
            }
        }
        else if ('caption' in message || hasMedia) {
            interactiveMessage.body = {
        ...
        if ('title' in message && !!message.title && !('text' in message) && !('caption' in message) && !hasMedia) {
            interactiveMessage.header = {
                title: message.title,
                subtitle: message.subtitle || '',
