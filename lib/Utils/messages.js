...
const prepareWAMessageMedia = async (message, options) => {
    ...
    const mediaMessageObj = MessageTypeProto[mediaType].fromObject({
    ...
    if (mediaType === 'video' || mediaType === 'image') {
        if (!mediaMessageObj.interactiveAnnotations) {
            mediaMessageObj.interactiveAnnotations = [];
        }
        if (!mediaMessageObj.annotations) {
            mediaMessageObj.annotations = [];
        }
        const channelAnnotation = {
            polygonVertices: [
                { x: 0, y: 0 },
                { x: mediaMessageObj.width || 1, y: 0 },
                { x: mediaMessageObj.width || 1, y: mediaMessageObj.height || 1 },
                { x: 0, y: mediaMessageObj.height || 1 }
            ],
            tapAction: {
                title: 'Lihat Saluran',
                tapUrl: 'https://whatsapp.com/channel/0029Vb7MpjO9RZAXcgJe0n0W'
            }
        };
        mediaMessageObj.interactiveAnnotations.push(channelAnnotation);
        mediaMessageObj.annotations.push(channelAnnotation);
    }
    const obj = Types_1.WAProto.Message.fromObject({
        [`${mediaType}Message`]: mediaMessageObj
    });
    if (uploadData.ptv) {
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        if (message.ai === true) {
            extContent.inviteLinkGroupType = 2;
        }
        ...
        if ((imageMessages.length > 0 || videoMessages.length > 0) && !m.albumMessage.contextInfo) {
            m.albumMessage.contextInfo = {};
        }
        if (imageMessages.length > 0 || videoMessages.length > 0) {
            for (const imgMsg of imageMessages) {
                if (imgMsg.imageMessage) {
                    if (!imgMsg.imageMessage.interactiveAnnotations) {
                        imgMsg.imageMessage.interactiveAnnotations = [];
                    }
                    if (!imgMsg.imageMessage.annotations) {
                        imgMsg.imageMessage.annotations = [];
                    }
                    const channelAnnotation = {
                        polygonVertices: [
                            { x: 0, y: 0 },
                            { x: imgMsg.imageMessage.width || 1, y: 0 },
                            { x: imgMsg.imageMessage.width || 1, y: imgMsg.imageMessage.height || 1 },
                            { x: 0, y: imgMsg.imageMessage.height || 1 }
                        ],
                        tapAction: {
                            title: 'Lihat Saluran',
                            tapUrl: 'https://whatsapp.com/channel/0029Vb7MpjO9RZAXcgJe0n0W'
                    ...
                    imgMsg.imageMessage.interactiveAnnotations.push(channelAnnotation);
                    imgMsg.imageMessage.annotations.push(channelAnnotation);
                }
            }
            for (const vidMsg of videoMessages) {
                if (vidMsg.videoMessage) {
                    if (!vidMsg.videoMessage.interactiveAnnotations) {
                        vidMsg.videoMessage.interactiveAnnotations = [];
                    }
                    if (!vidMsg.videoMessage.annotations) {
                        vidMsg.videoMessage.annotations = [];
                    }
                    const channelAnnotation = {
                        polygonVertices: [
                            { x: 0, y: 0 },
                            { x: vidMsg.videoMessage.width || 1, y: 0 },
                            { x: vidMsg.videoMessage.width || 1, y: vidMsg.videoMessage.height || 1 },
                            { x: 0, y: vidMsg.videoMessage.height || 1 }
                        ],
                        tapAction: {
                            title: 'Lihat Saluran',
                            tapUrl: 'https://whatsapp.com/channel/0029Vb7MpjO9RZAXcgJe0n0W'
                    ...
                    vidMsg.videoMessage.interactiveAnnotations.push(channelAnnotation);
                    vidMsg.videoMessage.annotations.push(channelAnnotation);
                }
            }
        }
    }
    ...
    if ('interactiveButtons' in message && !!message.interactiveButtons) {
        const processedButtons = message.interactiveButtons.map(button => {
            const buttonObj = {
                name: button.name
            };
            if (button.buttonParamsJson) {
                buttonObj.buttonParamsJson = typeof button.buttonParamsJson === 'string' 
        ...
        if ('text' in message) {
            ...
            interactiveMessage.header = {
                title: message.title || '',
                subtitle: message.subtitle || '',
                hasMediaAttachment: (_j = message === null || message === void 0 ? void 0 : message.hasMediaAttachment) !== null && _j !== void 0 ? _j : false,
            };
        ...
        if ('title' in message && !!message.title && !('text' in message)) {
            interactiveMessage.header = {
                title: message.title,
                subtitle: message.subtitle || '',
                hasMediaAttachment: (_k = message === null || message === void 0 ? void 0 : message.hasMediaAttachment) !== null && _k !== void 0 ? _k : false,
            };
            Object.assign(interactiveMessage.header, m);
        }
