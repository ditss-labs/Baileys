...
const downloadContentFromMessage = async ({ mediaKey, directPath, url }, type, opts = {}) => {
    if (mediaKey === null || mediaKey === undefined) {
        throw new boom_1.Boom('Media key is required to download encrypted content', { statusCode: 400 });
    }
    ...
    if (Buffer.isBuffer(mediaKey) && mediaKey.length === 0) {
        throw new boom_1.Boom('Media key is required to download encrypted content', { statusCode: 400 });
    }
    
    if (mediaKey instanceof Uint8Array && mediaKey.length === 0) {
        throw new boom_1.Boom('Media key is required to download encrypted content', { statusCode: 400 });
    }
