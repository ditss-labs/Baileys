...
async function getMediaKeys(buffer, mediaType) {
    if (!buffer || (Buffer.isBuffer(buffer) && buffer.length === 0) || (typeof buffer === 'string' && buffer.trim().length === 0)) {
...
const downloadContentFromMessage = async ({ mediaKey, directPath, url }, type, opts = {}) => {
    if (!mediaKey || (Buffer.isBuffer(mediaKey) && mediaKey.length === 0) || (typeof mediaKey === 'string' && mediaKey.trim().length === 0)) {
