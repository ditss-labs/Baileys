...
async function getMediaKeys(buffer, mediaType) {
    if (buffer === null || buffer === undefined) {
        throw new boom_1.Boom('Cannot derive from empty media key', { statusCode: 400 });
    }
    
    let mediaKeyBuffer = null;
    
    if (Buffer.isBuffer(buffer)) {
        mediaKeyBuffer = buffer;
    } else if (typeof buffer === 'string') {
        const base64Data = buffer.replace('data:;base64,', '').trim();
        if (!base64Data || base64Data.length === 0) {
            throw new boom_1.Boom('Cannot derive from empty media key', { statusCode: 400 });
        }
        try {
            mediaKeyBuffer = Buffer.from(base64Data, 'base64');
        } catch (error) {
            throw new boom_1.Boom('Invalid base64 media key format', { statusCode: 400 });
        }
    } else if (buffer instanceof Uint8Array) {
        mediaKeyBuffer = Buffer.from(buffer);
    } else if (Array.isArray(buffer)) {
        mediaKeyBuffer = Buffer.from(buffer);
    } else if (buffer && typeof buffer === 'object' && buffer.constructor && buffer.constructor.name === 'Uint8Array') {
        mediaKeyBuffer = Buffer.from(buffer);
    } else if (buffer && typeof buffer === 'object' && buffer.length !== undefined) {
        try {
            mediaKeyBuffer = Buffer.from(buffer);
        } catch (error) {
            throw new boom_1.Boom('Invalid media key buffer type', { statusCode: 400 });
        }
    } else {
        try {
            mediaKeyBuffer = Buffer.from(buffer);
    ...
    if (!mediaKeyBuffer || !Buffer.isBuffer(mediaKeyBuffer)) {
        throw new boom_1.Boom('Invalid media key buffer', { statusCode: 400 });
    }
    ...
    const expandedMediaKey = await (0, crypto_1.hkdf)(mediaKeyBuffer, 112, { info: (0, exports.hkdfInfoKey)(mediaType) });
    return {
...
const downloadContentFromMessage = async (mediaMessage = {}, type, opts = {}) => {
    var _a;
    const { mediaKey, directPath, url } = mediaMessage;
    const resolvedMediaKey = (_a = (mediaKey !== null && mediaKey !== void 0 ? mediaKey : opts.mediaKey)) !== null && _a !== void 0 ? _a : undefined;
    const isMmgUrl = typeof url === 'string' && url.includes(DEF_HOST);
    const shouldDecrypt = !!directPath || isMmgUrl;
    const hasPlainUrl = !!url && !shouldDecrypt;
    const normalizeMediaKey = (key) => {
        ...
        if (key instanceof Uint8Array) {
            return key.length ? Buffer.from(key) : undefined;
        }
        return key;
    };
    if (hasPlainUrl) {
        return (0, exports.getHttpStream)(url, opts === null || opts === void 0 ? void 0 : opts.options);
    }
    const normalizedMediaKey = normalizeMediaKey(resolvedMediaKey);
    if (!normalizedMediaKey && url && !isMmgUrl) {
        return (0, exports.getHttpStream)(url, opts === null || opts === void 0 ? void 0 : opts.options);
    }
    if (!normalizedMediaKey) {
        throw new boom_1.Boom('Media key is required to download encrypted content', {
            statusCode: 400,
            data: { directPath, url }
        });
    }
    const isValidMediaUrl = url === null || url === void 0 ? void 0 : url.startsWith(`https://${DEF_HOST}/`);
    const downloadUrl = isValidMediaUrl ? url : (0, exports.getUrlFromDirectPath)(directPath);
    if (!downloadUrl) {
        throw new boom_1.Boom('No valid media URL or directPath present in message', { statusCode: 400 });
    }
    try {
        const keys = await getMediaKeys(normalizedMediaKey, type);
        ...
        throw new boom_1.Boom(`Failed to process media key: ${error.message}`, { statusCode: 400, cause: error });
    }
};
