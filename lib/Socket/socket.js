...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairCode = "YEMOBYTE") => {
        if (!Object.keys(Defaults_1.PHONENUMBER_MCC).some(mcc => phoneNumber.startsWith(mcc))) {
            logger.error("phoneNumber format must be like: 62xxx (starts with country code)");
            throw new boom_1.Boom('Invalid phone number format. Must start with country code (e.g., 62xxx for Indonesia)', { statusCode: 400 });
        }
        if (pairCode) {
