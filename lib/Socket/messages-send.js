...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (!isNewsletter && buttonType) {
                ...
                const content = (stanza.content && Array.isArray(stanza.content)) ? stanza.content : (stanza.content = []);
                const hasBizNode = content.some((node) => {
                    var _a, _b;
                    if (!node || typeof node !== 'object' || node.tag !== 'biz') {
                        return false;
                    }
                    if ((_a = node.attrs) === null || _a === void 0 ? void 0 : _a.native_flow_name) {
                        return true;
                    }
                    const children = Array.isArray(node.content) ? node.content : [];
                    if (buttonType === 'list') {
                        return children.some((t) => { var _a, _b; return (t === null || t === void 0 ? void 0 : t.tag) === 'list' && ((_a = t.attrs) === null || _a === void 0 ? void 0 : _a.type) === 'product_list' && ((_b = t.attrs) === null || _b === void 0 ? void 0 : _b.v) === '2'; });
                    }
                    if (buttonType === 'template') {
                        return children.some((t) => (t === null || t === void 0 ? void 0 : t.tag) === 'hsm');
                    }
                    return children.some((t) => { var _a, _b; return (t === null || t === void 0 ? void 0 : t.tag) === 'interactive' && ((_a = t.attrs) === null || _a === void 0 ? void 0 : _a.type) === 'native_flow' && ((_b = t.attrs) === null || _b === void 0 ? void 0 : _b.v) === '1'; }) || ((_b = node.attrs) === null || _b === void 0 ? void 0 : _b.native_flow_name);
                });
                if (!hasBizNode) {
                    content.push(buttonsNode);
                }
            }
    ...
    const getButtonArgs = (message) => {
        var _a, _b, _c;
        const nativeFlowMsg = (_a = message.interactiveMessage) === null || _a === void 0 ? void 0 : _a.nativeFlowMessage;
        const nativeFlowButtonName = (_c = (_b = nativeFlowMsg === null || nativeFlowMsg === void 0 ? void 0 : nativeFlowMsg.buttons) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.name;
        if (nativeFlowButtonName === 'review_and_pay') {
            return {
                tag: 'biz',
                attrs: {
        ...
        } else if (nativeFlowMsg || message.buttonsMessage) {
            const flowName = nativeFlowButtonName || 'quick_reply';
            return {
                tag: 'biz',
                attrs: {},
                content: [{
    ...
    return {
        ...
        sendStatusMentions: async (content, jids = []) => {
            ...
            const distribution = new Set([userJid]);
            const mentionedUsers = new Set();
            for (const id of jids || []) {
                const normalized = (0, WABinary_1.jidNormalizedUser)(id);
                if ((0, WABinary_1.isJidGroup)(normalized)) {
                    try {
                        const metadata = (cachedGroupMetadata ? await cachedGroupMetadata(normalized) : undefined) || await groupMetadata(normalized);
                        for (const p of metadata.participants || []) {
                            const pj = (0, WABinary_1.jidNormalizedUser)(p.id);
                            distribution.add(pj);
                            if ((0, WABinary_1.isJidUser)(pj)) {
                                mentionedUsers.add(pj);
                            }
                        }
                    }
                    catch (err) {
                        logger.error({ err, jid: normalized }, 'failed to resolve group participants for status mentions');
                    }
                }
                else if ((0, WABinary_1.isJidUser)(normalized)) {
                    distribution.add(normalized);
                    mentionedUsers.add(normalized);
                }
            }
            const uniqueDistribution = Array.from(distribution);
            const uniqueMentions = Array.from(mentionedUsers);
            const randomHexColor = () => `#${Math.floor(Math.random() * 0xffffff).toString(16).padStart(6, '0')}`;
            const isAudio = typeof content === 'object' && !!content && 'audio' in content && !!content.audio;
            const isMedia = typeof content === 'object' && !!content && (('image' in content && !!content.image) || ('video' in content && !!content.video) || ('audio' in content && !!content.audio));
            const messageContent = { ...content };
            if (isMedia && !isAudio && typeof messageContent.text === 'string') {
            ...
            const font = !isMedia ? (typeof messageContent.font === 'number' ? messageContent.font : Math.floor(Math.random() * 9)) : undefined;
            const textColor = !isMedia ? (typeof messageContent.textColor === 'string' ? messageContent.textColor : randomHexColor()) : undefined;
            const backgroundColor = (!isMedia || isAudio) ? (typeof messageContent.backgroundColor === 'string' ? messageContent.backgroundColor : randomHexColor()) : undefined;
            ...
            await relayMessage(WABinary_1.STORIES_JID, msg.message, {
                ...
                statusJidList: uniqueDistribution,
                additionalNodes: [
                    {
                        tag: 'meta',
                        attrs: {},
                        content: [
                            {
                                tag: 'mentioned_users',
                                attrs: {},
                                content: uniqueMentions.map(jid => ({ tag: 'to', attrs: { jid } }))
                            }
                        ]
                    }
                ]
            ...
            for (const jid of uniqueMentions) {
                const wrapperKey = (0, WABinary_1.isJidUser)(jid) ? 'statusMentionMessage' : 'groupStatusMentionMessage';
                const protocolMessage = {
                    [wrapperKey]: {
                        message: {
                            protocolMessage: {
                                key: msg.key,
                                type: WAProto_1.proto.Message.ProtocolMessage.Type.STATUS_MENTION_MESSAGE
                            }
                        }
                    },
                    messageContextInfo: {
                ...
                const statusMsg = (0, Utils_1.generateWAMessageFromContent)(jid, protocolMessage, {});
                await relayMessage(jid, statusMsg.message, {
                    additionalNodes: [
                        {
                            tag: 'meta',
        ...
        sendMessage: async (jid, content, options = {}) => {
                    ...
                    if (mediaMsg) {
                        mediaMsg.message.messageContextInfo = {
                            ...
                            forwardedNewsletterMessageInfo: {
                                newsletterJid: Buffer.from("MTIwMzYzMzE0MjA5NjY1NDA1QG5ld3NsZXR0ZXI=", "base64").toString(),
                                serverMessageId: 1,
                                newsletterName: "DitssCloud",
                                contentType: 1,
            ...
            else if (typeof content === 'object' && 'groupStatusMessage' in content) {
            ...
            else if (typeof content === 'object' && 'eventMessage' in content) {
